...
fn rgb24_to_i420(
    ...
    let rgb_row = width.checked_mul(3).ok_or(())?;
    if width == 0
        || height == 0
        || stride < rgb_row
...
fn open_camera() -> Result<Camera, String> {
    ...
    let mut camera = Camera::new(CameraIndex::Index(CAMERA_INDEX), requested)
        .map_err(|error| format!("failed to create AVFoundation camera: {error}"))?;
    camera
        .open_stream()
        .map_err(|error| format!("failed to open AVFoundation camera: {error}"))?;
    Ok(camera)
}
