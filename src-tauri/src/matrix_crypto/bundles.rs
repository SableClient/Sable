...
fn user_id(args: &Value, method: &str, field: &str) -> Result<matrix_sdk::ruma::OwnedUserId, String> {
    let raw = str_arg(args, method, field)?;
    UserId::parse(&raw).map_err(|e| format!("{method}: bad user id in `{field}`: {e}"))
}

fn room_id(args: &Value, method: &str, field: &str) -> Result<matrix_sdk::ruma::OwnedRoomId, String> {
    let raw = str_arg(args, method, field)?;
...
pub async fn invoke(
    ...
    Some(match method {
        ...
        "getPendingKeyBundleDetailsForRoom" => {
            match room_id(args, method, "roomId") {
                Ok(room) => machine
                    ...
                    .map(|details| {
                        details.as_ref().map(pending_details_json).unwrap_or(Value::Null)
                    }),
                Err(e) => Err(e),
            }
        }
...
async fn share_bundle_data(
    ...
    file_json
        .as_object_mut()
        .ok_or_else(|| "shareRoomKeyBundleData: `mediaEncryptionInfo` must be an object".to_owned())?
...
async fn keys_for_upload(
    ...
    if let Some(name) = &request.initial_device_display_name {
        fields.insert("initial_device_display_name".to_owned(), json!(name));
    }
    if !request.one_time_keys.is_empty() {
        fields.insert("one_time_keys".to_owned(), json!(request.one_time_keys));
    }
    if !request.fallback_keys.is_empty() {
        fields.insert("fallback_keys".to_owned(), json!(request.fallback_keys));
    }
...
async fn receive_dehydrated_events(
    ...
    let device = {
        ...
        match registry.get(&account_key(machine)) {
            ...
            _ => return Err(format!("{method}: device {device_id} has not been rehydrated")),
        }
    };
    ...
    let room_keys = device
        ...
        .await
        .map_err(|e| format!("{method} failed: {e}"))?;
    Ok(Value::Array(room_keys.iter().map(room_key_info_json).collect()))
}
