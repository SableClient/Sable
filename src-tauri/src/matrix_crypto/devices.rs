...
async fn set_local_trust(machine: &OlmMachine, args: &Value, method: &str) -> Result<Value, String> {
    let trust = args
        .get("trustState")
...
async fn verify_identity(machine: &OlmMachine, args: &Value, method: &str) -> Result<Value, String> {
