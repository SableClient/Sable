...
type IncomingVerificationDialogProps = {
...
export function IncomingVerificationDialog({
  ...
  const [otherUserId, setOtherUserId] = useState<string | undefined>(request?.otherUserId);

  // Listen for phase changes on the request
  useEffect(() => {
    const onChange = () => {
      setPhase(request.phase);
      // Update otherUserId if it changes
      if (request.otherUserId !== otherUserId) {
  ...
  const otherUser = otherUserId ? mx.getUser(otherUserId) : null;
  const displayName = otherUser?.displayName || otherUserId || 'Unknown user';
          ...
          <Dialog variant="Surface">
            ...
            <Box style={{ padding: config.space.S400 }} direction="Column" gap="400">
              {phase === VerificationPhase.Requested && (
                <Box direction="Column" gap="400">
                  <Text>{t('verification.verify_request_from', { user: displayName })}</Text>
...
type IncomingVerificationProviderProps = {
  children: ReactNode;
};

export function IncomingVerificationProvider({ children }: IncomingVerificationProviderProps) {
  const mx = useMatrixClient();
  ...
  useEffect(() => {
    const handler = (req: VerificationRequest) => {
      // Only handle user-to-user verification (not self-verification)
      if (req.isSelfVerification) return;
      setRequest(req);
      setOtherUserId(req.otherUserId);
    };
    mx.on(CryptoEvent.VerificationRequestReceived, handler);
    return () => {
      mx.removeListener(CryptoEvent.VerificationRequestReceived, handler);
    };
  }, [mx]);
