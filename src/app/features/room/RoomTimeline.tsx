...
// Returns the number of replies in a thread, preferring thread.events over the
// server-aggregated count (which may be 0 on homeservers without thread support).
const getThreadReplyCount = (room: Room, mEventId: string): number => {
  const thread = room.getThread(mEventId);
  if (thread) {
    const fromEvents = thread.events.filter((ev) => ev.getId() !== mEventId).length;
    return fromEvents > 0 ? fromEvents : thread.length;
  }
  return room
    .getUnfilteredTimelineSet()
    .getLiveTimeline()
    .getEvents()
    .filter((ev) => ev.threadRootId === mEventId).length;
};
...
function ThreadReplyChip({ room, mEventId, openThreadId, onToggle }: ThreadReplyChipProps) {
  ...
  const replyEvents = (() => {
    if (thread) {
      const fromThread = thread.events.filter((ev) => ev.getId() !== mEventId);
  ...
  const replyCount = replyEvents.length > 0 ? replyEvents.length : (thread?.length ?? 0);
...
export function RoomTimeline({
  ...
  const eventRenderer = (item: number) => {
    ...
    // main room timeline (they may be present as local echoes or because they
    // were first synced before threadSupport was enabled).
    if (mEvent.threadRootId !== undefined) return null;
