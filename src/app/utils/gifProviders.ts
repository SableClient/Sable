...
export type GifProvider = {
  ...
  /** Payload the soliditas proxy expects after the `<id>_` prefix. */
  getProxyPayload: (url: URL, gif: GifData) => string | undefined;
  /** What the proxy actually serves, which is not always the picked rendition. */
...
export const GIF_PROVIDERS: Record<GifProviderId, GifProvider> = {
  ...
  tenor: {
    ...
    getProxyPayload: (url) => url.pathname.split('/').find(Boolean),
    proxyMimetype: 'image/gif',
  },
  giphy: {
    id: 'giphy',
    ...
    // The proxy serves i.giphy.com/<id>.webp, so it needs the media id, not the rendition path.
    getProxyPayload: (url, gif) => {
      const segments = url.pathname.split('/').filter(Boolean);
      const fromPath = segments.at(-2) ?? segments.at(-1)?.replace(/\.\w+$/, '');
      return gif.id || fromPath;
    },
...
/** Mints the `mxc://<proxy>/<provider>_<base64url(payload)>` URL soliditas resolves. */
